...
void callback(char* topic, byte* payload, unsigned int length) 
{
  ...
  if (str =="bozaSub/kuca/node1/prskalica1"){
    ...
    digitalWrite(PRSKALICA2, !da);
    digitalWrite(PRSKALICA3, !da);
    digitalWrite(PRSKALICA4, !da);
  }
  ...
  //fontana pumpa
    ...
    if(p=='s'){
      ...
      if(digitalRead(PRSKALICA1)==da) Stanje[0]='1';
      else Stanje[0]='0';
      if(digitalRead(PRSKALICA2)==da) Stanje[1]='1';
      else Stanje[1]='0';
      if(digitalRead(PRSKALICA3)==da) Stanje[2]='1';
      else Stanje[2]='0';
      if(digitalRead(PRSKALICA4)==da) Stanje[3]='1';
      else Stanje[3]='0';
      if(digitalRead(FONTANAPUMPA)==da) Stanje[4]='1';
      else Stanje[4]='0';
      if(digitalRead(RADIO)==da) Stanje[5]='1';
      else Stanje[5]='0';
      if(digitalRead(NEODREDJENO)==da) Stanje[6]='1';
      else Stanje[6]='0';
  ...
  if (digitalRead(PRSKALICA_ULAZ) == HIGH) {
    ...
    if ((millis() - buttonTimer1 > longPressTime) && (longPressActive1 == false)) {
      longPressActive1 = true;   
        if(digitalRead(PRSKALICA1) == !da && digitalRead(PRSKALICA2) == !da && digitalRead(PRSKALICA3) == !da && digitalRead(PRSKALICA4) == !da){
          prskalica = true;
  ...
  if (digitalRead(FONTANAPUMPA_ULAZ) == HIGH) {
    ...
    if ((millis() - buttonTimer2 > longPressTime) && (longPressActive2 == false)) {
      longPressActive2 = true;   
        if(digitalRead(FONTANAPUMPA)==!da){
          client.publish("bozaSub/kuca/node1/fontanaPumpa/stanje", String("1").c_str(),false);
          digitalWrite(FONTANAPUMPA, da);
        }
        else{
